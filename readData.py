...
fractionOrder = np.char.split(fractionOrder, '_')

temp = np.array([])
for i in range(0, len(fractionOrder)):
    temp = np.append(temp, fractionOrder[i][1])

fractionOrder = temp

print(fractionOrder)

heightOrder = np.char.split(heightOrder, '_')
temp = np.array([])
for i in range(0, len(heightOrder)):
    temp = np.append(temp, heightOrder[i][2][0:-4])
...
plt.contourf(X, Y, Z1, 200, cmap='CMRmap')
plt.title('k-eff')
plt.xlabel('height')
...
plt.contourf(X, Y, Z2, 200, cmap='CMRmap')
